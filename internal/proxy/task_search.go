...
type searchTask struct {
	...
	*internalpb.SearchRequest
	...
	// Semantic Pyramid Indexing (SPI) runtime state.
	spiConfig     *semanticPyramidConfig
	spiBaseParams map[string]any
	spiRuntime    *semanticPyramidRuntime
	planBase      *planpb.PlanNode
	originalTopK  int64
}
...
func (t *searchTask) PreExecute(ctx context.Context) error {
	...
	if t.originalTopK == 0 {
		t.originalTopK = t.request.GetTopk()
	}
...
func (t *searchTask) initSearchRequest(ctx context.Context) error {
	...
	if cfg, params, err := parseSemanticPyramidConfig(queryInfo.GetSearchParams()); err != nil {
		return err
	} else {
		t.spiConfig = cfg
		t.spiBaseParams = params
		if params != nil {
			encoded, err := encodeSearchParams(params)
			if err != nil {
				return err
			}
			queryInfo.SearchParams = encoded
			if planQueryInfo, err := extractPlanQueryInfo(plan); err == nil {
				planQueryInfo.SearchParams = encoded
			} else {
				return err
			}
		}
	...
	if candidateTopk := queryInfo.GetTopk() - offset; candidateTopk > 0 {
		t.originalTopK = candidateTopk
	}
	...
	t.planBase = proto.Clone(plan).(*planpb.PlanNode)
	...
	if err := t.prepareSemanticPyramid(plan, queryInfo); err != nil {
		return err
	}
