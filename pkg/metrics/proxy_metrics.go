...
var (
	...
	// ProxySPIDecisions records semantic pyramid level selections.
	ProxySPIDecisions = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: milvusNamespace,
			Subsystem: typeutil.ProxyRole,
			Name:      "search_spi_level_total",
			Help:      "counter of semantic pyramid level selections",
		}, []string{nodeIDLabelName, collectionName, "spi_level"})
...
func RegisterProxy(registry *prometheus.Registry) {
	...
	registry.MustRegister(ProxySPIDecisions)
